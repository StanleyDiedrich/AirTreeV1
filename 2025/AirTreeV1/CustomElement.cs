        ...
        public CustomElement(Autodesk.Revit.DB.Document doc, ElementId elementId)
        {
            ...
            if (Element is Duct)
            {
                ...
                foreach (Connector connector in OwnConnectors)
                {
                    ...
                    if (Element.Category.Id.Value == (int)BuiltInCategory.OST_DuctInsulations)
                    {
                        continue;
                    }
                    if (doc.GetElement(connector.Owner.Id).Category.Id.Value== -2008015) // Механическая система
                    {
                        continue;
                    }    
                            ...
                            else
                            {
                                ...
                                else if (ShortSystemName.Contains(ShortSystemName))
                                {
                                    if (connect.Domain == Autodesk.Revit.DB.Domain.DomainHvac || connect.Domain == Autodesk.Revit.DB.Domain.DomainPiping)
                                    {
                                        ...
                                        {
                                            if (doc.GetElement(connect.Owner.Id).Category.Id.Value == (int)BuiltInCategory.OST_DuctInsulations)
                                            {
                                                continue;
                                            }
                                            if (doc.GetElement(connect.Owner.Id).Category.Id.Value == -2008015) // Механическая система
                                            {
                                                continue;
                                            }
            ...
            if (Element is FamilyInstance)
            {
                ...
                else if (Element.Category.Id.Value == -2008016)
                {
                    DetailType = Detail.FireProtectValve;


                }
                else if (Element.Category.Id.Value == -2008013)
                {
                    DetailType = Detail.AirTerminal;
                }
                else if (Element.Category.Id.Value == -2001140)
                {
